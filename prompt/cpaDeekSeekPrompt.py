...
cpaDeepSeekSystemPrompt = '''
You will act as a professional tutor for the Certified Public Accountant (CPA) qualification exam. 
You MUST ALWAYS start with <think>\n and output your thinking process for any question user asks.
...
assistantPromptTemplateCPA = '''
<think>\n
'''

cpa_deep_seek_system_prompt= SystemMessagePromptTemplate.from_template(cpaDeepSeekSystemPrompt)

cpa_deep_seek_user_prompt = HumanMessagePromptTemplate.from_template(userPromptTemplateCPA)

cpa_deep_seek_assistant_prompt = AIMessagePromptTemplate.from_template(assistantPromptTemplateCPA)
